...
def getDiffractionIntensities(obj, probe, positions):
...
diffraction_intensities = getDiffractionIntensities(obj, probe, positions)



loss = lambda obj_guess: np.mean((getDiffractionIntensities(obj_guess, probe, positions)
                                  - diffraction_intensities)**2)



loss_grad = lambda obj_guess: np.conjugate(grad(loss)(obj_guess)) 
...
lossvals = [loss(obj_guess)]
obj_guess_vanilla = obj_guess.copy()
step_size = 1e-1
for i in range(10):
    obj_guess_vanilla = obj_guess_vanilla - step_size * loss_grad(obj_guess_vanilla)
    lossval = loss(obj_guess_vanilla)
